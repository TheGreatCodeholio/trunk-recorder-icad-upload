...
public:
  ...
  int call_end(Call_Data_t call_info) override
  {
    ...
    nlohmann::ordered_json call_json = {
        ...
        {"sys_name", call_info.short_name},
        {"freq", call_info.freq},
        {"freqList", call_info.call_json.freqList},
        {"srcList", call_info.call_json.srcList},
        {"talkgroup", call_info.talkgroup},
        {"talkgroup_alpha_tag", call_info.talkgroup_alpha_tag},
        {"talkgroup_description", call_info.talkgroup_description},
        {"talkgroup_group", call_info.talkgroup_group},
  ...
  int send_file(const std::string& filename, const std::string& object_topic) {
      if (!mqtt_connected) {
          BOOST_LOG_TRIVIAL(error) << "MQTT not connected";
          return 0;
      }
      ...
      // Assemble the MQTT message with binary payload
      mqtt::message_ptr pubmsg = mqtt::message_ptr_builder()
              .topic(object_topic + "/audio")
              .payload(file_data.data(), file_data.size())
              .qos(mqtt_qos)
              .retained(false)
              .finalize();
      ...
      try {
          ...
          BOOST_LOG_TRIVIAL(error) << log_prefix << exc.what();
          return 0;
      }
      return 1;
  }
